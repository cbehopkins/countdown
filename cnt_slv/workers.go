...
func workN(arrayIn NumCol, foundValues *NumMap) SolLst {
	return workNmod(arrayIn, foundValues)
}
...
func workNmod(arrayIn NumCol, foundValues *NumMap) SolLst {
	...
	if lenArrayIn <= 1 {
		...
		tmpList = foundValues.make2To1mod(arrayIn[0:2])
		...
		tmpList = foundValues.make3To1mod(arrayIn[0:3])
		foundValues.addMany(tmpList...)
		// and add the origioanl list on
		return append(SolLst{}, tmpList, arrayIn)
	}
...
func workNold(arrayIn NumCol, foundValues *NumMap) SolLst {
	var retList SolLst
	lenArrayIn := arrayIn.Len()
	if foundValues.Solved() {
		return retList
